...
<body>
    <header>
        <div class="header-content max-w-7xl mx-auto flex justify-between items-center">
            <div class="logo">
                <img src="Amen1.png" alt="SolarCalc Logo" width="50" height="50" onerror="this.src='https://via.placeholder.com/50'">
    ...
    <div class="container max-w-4xl mx-auto p-4">
        ...
        <form id="sizing-form">
            ...
            <div class="form-group">
                <label class="block text-sm font-medium">Coût Onduleur (CFA/onduleur)</label>
                    ...
                    <table class="w-full border-collapse mt-2">
                        <thead>
                            <tr class="bg-gray-100">
                                ...
                                <th class="border p-2">Coût (CFA)</th>
                            </tr>
                        </thead>
...
async function generatePDF() {
    try {
        ...
        const formData = {
            ...
            'Type de Système': form.querySelector('#system-type').value.charAt(0).toUpperCase() + form.querySelector('#system-type').value.slice(1) || 'Non spécifié',
            'Tension Système': `${form.querySelector('[name="system-voltage"]')?.value || 'N/A'} V`,
            'Type de Panneau': form.querySelector('#panel-type').value.charAt(0).toUpperCase() + form.querySelector('#panel-type').value.slice(1) || 'Non spécifié',
            ...
            'Type de Batterie': form.querySelector('[name="battery-type"]')?.value.charAt(0).toUpperCase() + form.querySelector('[name="battery-type"]')?.value.slice(1) || 'N/A',
        ...
        const applianceData = Array.from(appliances).map((appliance, index) => {
            return [
                appliance.querySelector(`[name="appliance[${index}][name]"]`).value.charAt(0).toUpperCase() + appliance.querySelector(`[name="appliance[${index}][name]"]`).value.slice(1),
        ...
        if (results) {
            ...
            const resultsData = [
                ...
                ['Type de Système', systemType.charAt(0).toUpperCase() + systemType.slice(1)],
        ...
        for (const chart of charts) {
            ...
            if (canvas) {
                ...
                const imgData = await html2canvas(canvas, { scale: 2 }).then(canvas => canvas.toDataURL('image/png', 0.8));
                const imgWidth = 170;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelector('.theme-toggle')?.addEventListener('click', (e) => {
        ...
        if (body) {
            ...
            document.querySelectorAll('#sizing-form, .chart-container').forEach(el => {
                if (el) {
...
function calculateOptimalTilt(latitude) {
    if (latitude >= 0) { // Hémisphère nord
        return Math.round(latitude * 0.9);
    } else { // Hémisphère sud
